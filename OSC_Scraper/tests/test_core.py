...
# --------------------------------------------------------- seleção da base API
def test_probe_base_prefers_working_search_endpoint():
    """Regressão: a base é escolhida validando o endpoint de BUSCA.

    Antes, ``probe_base`` checava ``/osc/dados_gerais/1`` e podia escolher uma
    base que dava **404 na busca avançada** (ex.: ``…/novomapaosc/api/api``,
    com o segmento ``api/api`` duplicado) — o que zerava as OSCs encontradas.
    Agora a base só é aceita se a busca realmente devolver OSCs, então a base
    quebrada é rejeitada mesmo vindo primeiro na lista.
    """
    import httpx as _httpx

    from api_client import APIClient

    good = "https://mapaosc.ipea.gov.br/api"
    broken = "https://mapaosc.ipea.gov.br/novomapaosc/api/api"

    def handler(request: _httpx.Request) -> _httpx.Response:
        url = str(request.url)
        if url.startswith(broken):
            return _httpx.Response(404, text="Not Found")
        if url.startswith(good) and "busca_avancada" in url:
            return _httpx.Response(
                200, json=[{"id_osc": 1, "tx_nome_osc": "OSC de Teste"}]
            )
        return _httpx.Response(404, text="Not Found")
    ...
    async def run() -> str | None:
        ...
        try:
            # A base quebrada vem primeiro de propósito: ainda assim deve ser
            # descartada, e a base cuja busca funciona deve ser escolhida.
            return await client.probe_base([broken, good])
        finally:
