...
class EstoqueReposicaoTest {
    ...
    private String tenant(String doc) throws Exception {
        return body(mvc.perform(post("/api/v1/empresas")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"nome\":\"Kairós\",\"documento\":\"" + doc + "\"}"))
    ...
    void entradaElevaSaldoEReposicaoListaItemAbaixoDoPonto() throws Exception {
        ...
        mvc.perform(get("/api/v1/estoque/itens/reposicao").header(TENANT_HEADER, t))
    ...
    void listagemIsolaPorTenant() throws Exception {
        String a = tenant("44.444.444/0001-44");
        String b = tenant("55.555.555/0001-55");

        mvc.perform(post("/api/v1/estoque/itens").header(TENANT_HEADER, a)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"sku\":\"X\",\"descricao\":\"X\",\"saldoInicial\":1,\"pontoReposicao\":0}"))
                .andExpect(status().isCreated());

        // Tenant B não enxerga itens do tenant A
        mvc.perform(get("/api/v1/estoque/itens").header(TENANT_HEADER, b))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(0));
    }
}
