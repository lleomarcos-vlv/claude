...
- **SQLite** (via `sql.js`, SQLite em WebAssembly — 100% JavaScript, sem
  compilador nativo) — banco de dados local, criado automaticamente.
...
- **SQLite**, arquivo `database/estacao.db`, criado **automaticamente** na
  primeira execução. É um arquivo SQLite padrão, legível por qualquer
  ferramenta SQLite.
- Motor **`sql.js`** (SQLite compilado para WebAssembly): 100% JavaScript, sem
  compilador nativo. O arquivo é gravado em disco automaticamente a cada
  alteração.
- Usa **índices**, **relacionamentos** (chaves estrangeiras) e **triggers**
...
- **"Falha ao instalar as dependências"** — o banco usa `sql.js` (SQLite em
  WebAssembly), então **não é necessário** compilador nem "Visual Studio Build
  Tools". A causa quase sempre é acesso à internet no momento do download
  (rede, firewall, antivírus ou proxy). Verifique a conexão e rode
  `instalar.bat` novamente. Se persistir, instale o Node.js **LTS** e tente de
  outra rede.
