...
public class EquipamentoController {
    ...
    public record EquipamentoResponse(String id, String serialNumber, String modelo,
                                      ...
                                      String posseTipo, String posseNome) {
        static EquipamentoResponse of(Equipamento e) {
            return new EquipamentoResponse(e.getId(), e.getSerialNumber(), e.getModelo(),
                    e.getFabricante(), e.getStatus(), e.getPosseTipo(), e.getPosseNome());
        }
    }

    public record TransferirPosseRequest(
            @NotBlank String paraTipo,
            @NotBlank String paraNome,
            String observacao) {
    }
    ...
    public EquipamentoResponse transferir(@PathVariable String id,
                                          @Valid @RequestBody TransferirPosseRequest req) {
        return EquipamentoResponse.of(
                service.transferir(id, req.paraTipo(), req.paraNome(), req.observacao()));
    }
